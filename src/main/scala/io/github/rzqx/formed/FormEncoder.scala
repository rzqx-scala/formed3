...
object FormEncoder {
  ...
  final inline def derived[A](using mirror: Mirror.Of[A]): FormEncoder[A] =
    new FormEncoder[A] {
      ...
      final def encodeElemAt(index: Int, elem: Any): Chain[(Chain[String], String)] = {
      ...
      override def chained(value: A): Chain[(Chain[String], String)] = {
        ...
        iterable.foldLeft(Chain.empty[(Chain[String], String)]) { case (acc, chain) =>
          acc ++ chain
        }
      }
    }

  final inline def derive[A: Mirror.Of]: FormEncoder[A] = derived[A]
}
